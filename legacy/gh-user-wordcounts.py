...
users = sqlContext.sql("SELECT distinct(user.id_str) as user_id, count(user.id_str) as tweet_count FROM tweets GROUP BY user.id_str ORDER BY tweet_count DESC")
